...
class WC_Shortcode_My_Account {
	...
	private static function view_order( $order_id ) {
		...
		if ( ! current_user_can( 'view_order', $order_id ) ) {
			echo '<div class="woocommerce-error col-xs-12">' . __( 'Invalid order.', 'woocommerce' ) . ' <a href="' . wc_get_page_permalink( 'myaccount' ).'" class="wc-forward">'. __( 'My Account', 'woocommerce' ) .'</a>' . '</div>';
			return;
		}
